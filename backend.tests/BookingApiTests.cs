...
public sealed class BookingApiTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    {
        var adminTypes = await client.GetFromJsonAsync<EventTypeDto[]>("/admin/event-types");
        var publicTypes = await client.GetFromJsonAsync<EventTypeDto[]>("/event-types");
    ...
    public async Task CreateEventTypeReturnsCreatedModel()
    {
        ...
        var response = await client.PostAsJsonAsync("/admin/event-types", request);
    ...
    {
        var slots = await client.GetFromJsonAsync<SlotDto[]>("/event-types/intro-call/slots");
    ...
    {
        await client.PostAsJsonAsync("/admin/event-types", new CreateEventTypeRequestDto(
        ...
        var firstResponse = await client.PostAsJsonAsync("/bookings", new CreateBookingRequestDto(
            "intro-call",
            selected.StartAt,
        ...
        var secondResponse = await client.PostAsJsonAsync("/bookings", new CreateBookingRequestDto(
            "same-time-other-type",
            selected.StartAt,
    ...
    {
        var response = await client.GetAsync("/event-types/missing/slots");
    ...
    {
        var response = await client.PostAsJsonAsync("/bookings", new CreateBookingRequestDto(
            "intro-call",
    ...
    {
        var slots = await client.GetFromJsonAsync<SlotDto[]>("/event-types/consultation/slots");
        var selected = Assert.Single(slots!.Take(1));
        var createResponse = await client.PostAsJsonAsync("/bookings", new CreateBookingRequestDto(
            "consultation",
            selected.StartAt,
            new GuestDto("Maria", "maria@example.com")));
        createResponse.EnsureSuccessStatusCode();

        var bookings = await client.GetFromJsonAsync<BookingDto[]>("/admin/bookings/upcoming");
